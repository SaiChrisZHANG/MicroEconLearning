...
\begin{theorem} If $S(\mathbf{p},w)$ is negative semidefinite, then
    \begin{enumerate}
        ...
        \item[(c)] If $\mathbf{x}(\mathbf{p},w)$ is differentiable, homogeneous of degree 0 and satisfies Walras' law, then $\forall (\mathbf{p},w), \mathbf{p}\cdot S(\mathbf{p},w)=0, S(\mathbf{p},w)\mathbf{p}=0.$
        It is easy to prove: 
