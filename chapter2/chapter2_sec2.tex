...
$$\mathrm{D}_{\alpha \mathbf{p}}\mathbf{x}(\alpha \mathbf{p},\alpha w)\cdot \mathbf{p} + \mathrm{D}_{\alpha w} \mathbf{x}(\alpha \mathbf{p},\alpha w)w=0$$
this is true for any $\alpha$, if we take $\alpha=1$, we get $\mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p}, w)\cdot \mathbf{p} + \mathrm{D}_{w} \mathbf{x}( \mathbf{p}, w)w=\mathbf{0}$, which is just the matrix notation of Thm.\ref{thm:homo_waldemand_thm1}.
...
The proof is also easy: Walras' law gives $\mathbf{p}\cdot \mathbf{x}(\mathbf{p},w)=w$, take derivatives with respect to $\mathbf{p}$, get $\mathbf{p}\cdot \mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)+\mathbf{x}(\mathbf{p},w)^T=\mathbf{0}^T$\footnote{This will give: $\mathbf{p}\cdot \mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)\mathbf{p}+\mathbf{x}(\mathbf{p},w)^T\mathbf{p}=\mathbf{0}^T\Rightarrow \mathbf{p}\cdot \mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)\mathbf{p}=-w$}; take derivatives with repsect to $w$, get $\mathbf{p}\cdot \mathrm{D}_{w}(\mathbf{p},w)=1$. 
...
\begin{definition}{WARP of $\mathbf{x}(\mathbf{p},w)$}{walrasian_WARP}
...
Notice that so far, we have only linked WARP to \textbf{compensated} price changes, which means that we are focusing on the case that the budget line rotates through one point (not the end point) $\mathbf{x}(\mathbf{p},w)$ from $(\mathbf{p},w)$ to $(\mathbf{p}',w')$. 
...
In fact, if $\mathbf{x}(\cdot,\cdot)$ is homogeneous of degree one w.r.t. $w$, then the law of demand actually still holds for \textit{uncompensated} price changes: $\mathrm{d}p\cdot\mathrm{D}_\mathbf{p}\mathbf{x}(p,w)\mathrm{d}\mathbf{p}\leq 0,\forall \mathrm{d}\mathbf{p}$. A brief proof is: by homogeneity of degree one w.r.t. w, $\mathbf{x}(\mathbf{p},\alpha w)=\alpha \mathbf{x}(\mathbf{p},w)$,
differentiate w.r.t. $\alpha$ and evaluate at $\alpha =1$, get $\mathrm{D}_w \mathbf{x}(\mathbf{p}, w)w =\mathbf{x}(\mathbf{p},w)$, then the Slutsky matrix is $S(\mathbf{p},w)=\mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)+\mathrm{D}_w\mathbf{x}(\mathbf{p},w)\cdot \mathbf{x}(\mathbf{p},w)^T=\mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)+\frac{1}{w}\mathbf{x}(\mathbf{p},w)\mathbf{x}(\mathbf{p},w)^T$.
Hence $\mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)=S(\mathbf{p},w)-\frac{1}{w}\mathbf{x}(\mathbf{p},w)\mathbf{x}(\mathbf{p},w)^T$ is a negative semidefinite matrix, the proof is complete.}

\subsubsection*{WARP and multivalued $\mathrm{x}(\mathrm{p},w)$}
Here, we (briefly) extended WARP into multivalued $\mathbf{x}(\mathbf{p},w)$, Walrasian demand \textbf{correspondences}, which is a generalization of single-valued demand functions.

First, we have the generalized version of WARP:
\begin{definition}{generalized WARP of $\mathbf{x}(\mathbf{p},w)$}{walrasian_WARP_general}
    (Generalizing \ref{def:walrasian_WARP}) For any $(\mathbf{p},w)$ and $(\mathbf{p'},w')$, if $\mathbf{x}\in\mathbf{x}(\mathbf{p},w)$, $\mathbf{x}'\in\mathbf{x}(\mathbf{p}',w'), \mathbf{p}'\cdot \mathbf{x}\leq w'$ and $\mathbf{x}'\notin \mathbf{x}(\mathbf{p},w)$ lead to $\mathbf{p}\cdot\mathbf{x}' > w$, then the Walrasian demand correspondence satisfies WARP.
\end{definition}
...
\begin{theorem}{generalized compensated law of demand}{compensated_lawofdemand_general}
    (Generalizing \ref{thm:compensated_lawofdemand}) From any initial position $(\mathbf{p},w)$ with demand $\mathbf{x}\in \mathbf{x}(\mathbf{p},w)$ for compensated price change to new prices $\mathbf{p}'$ and wealth level $w'=\mathbf{p}'\cdots \mathbf{x}$, we have
    $$ (\mathbf{p}'-\mathbf{p})\cdot(\mathbf{x}'-\mathbf{x})\begin{cases}
        < 0, & \mathbf{x}'\notin\mathbf{x}(\mathbf{p},w)\\
        =0, & \mathbf{x}'\in\mathbf{x}(\mathbf{p},w)
    \end{cases}$$
\end{theorem}

Again, if the multivalued demand correspondence $\mathbf{x}(\mathbf{p},w)$ satisfies Walras' law and compensated law of demand (Def.\ref{thm:compensated_lawofdemand_general}), this $\mathbf{x}(\mathbf{p},w)$ satisfies the generalized WARP (Def.\ref{def:walrasian_WARP_general}), the proof follows the same logic of the single-valued version:
We only need to show for all compensated price changes, generalized WARP holds. We can prove its contrapositive, if WARP does not hold, there exists a compensated price violates the generalized compensated law of demand. Suppose $\mathbf{x}\in \mathbf{x}(\mathbf{p},w),\mathbf{x}'\in \mathbf{x}(\mathbf{p}',w')$, $\mathbf{p}'\cdot\mathbf{x}=w'$ and $\mathbf{p}\cdot\mathbf{x}'\leq w$, then it is easy to show that $(\mathbf{p}'-\mathbf{p})\cdot (\mathbf{x}'-\mathbf{x})=w-\mathbf{p}\cdot \mathbf{x}'\geq 0$, it violates the compensated law of demand, except for the case that $\mathbf{x}'\in \mathbf{x}(\mathbf{p},w)$, where $(\mathbf{p}'-\mathbf{p})\cdot (\mathbf{x}'-\mathbf{x})=0$.
...
If $S(\mathbf{p},w)$ is negative semidefinite, then
    \begin{enumerate}
        ...
        Thm.\ref{thm:walraslaw_waldemand_thm2} gives $\mathbf{p}\cdot S(\mathbf{p},w)=\mathbf{p}\cdot \mdiff_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)+\mathbf{p}\cdot \mdiff_w \mathbf{x}(\mathbf{p},w)\mathbf{x}(\mathbf{p},w)^T =\mathbf{p}\mdiff_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)+\mathbf{x}(\mathbf{p},w)^T=0$;
        Thm.\ref{thm:homo_waldemand_thm1} gives $S(\mathbf{p},w)\mathbf{p}=\mdiff_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)\mathbf{p}+\mdiff_w\mathbf{x}(\mathbf{p},w)\mathbf{x}(\mathbf{p},w)^T\mathbf{p}=\mdiff_{\mathbf{p}}\mathbf{x}(\mathbf{p},w)\mathbf{p}+\mdiff_w\mathbf{x}(\mathbf{p},w)w=0$.
    \end{enumerate}
\end{theorem}
