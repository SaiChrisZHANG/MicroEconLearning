...
this is true for any $\alpha$, if we take $\alpha=1$, we get $\mathrm{D}_{\mathbf{p}}\mathbf{x}(\mathbf{p}, w)\cdot \mathbf{p} + \mathrm{D}_{w} \mathbf{x}( \mathbf{p}, w)w=\mathbf{0}$, which is just the matrix notation of Thm.\ref{thm_homo_waldemand_thm1}.
...
\begin{definition}\label{def_walrasian_WARP_general}
    (Generalizing \ref{def_walrasian_WARP}) For any $(\mathbf{p},w)$ and $(\mathbf{p'},w')$, if $\mathbf{x}\in\mathbf{x}(\mathbf{p},w)$, $\mathbf{x}'\in\mathbf{x}(\mathbf{p}',w'), \mathbf{p}'\cdot \mathbf{x}\leq w'$ and $\mathbf{x}'\notin \mathbf{x}(\mathbf{p},w)$ lead to $\mathbf{p}\cdot\mathbf{x}' > w$, then the Walrasian demand correspondence satisfies WARP.
\end{definition}

and we have a generalized version of the compensated law of demand:
\begin{theorem}\label{thm_compensated_lawofdemand_general}
    (Generalizing \ref{thm_compensated_lawofdemand}) From any initial position $(\mathbf{p},w)$ with demand $\mathbf{x}\in \mathbf{x}(\mathbf{p},w)$ for compensated price change to new prices $\mathbf{p}'$ and wealth level $w'=\mathbf{p}'\cdots \mathbf{x}$, we have
...
Again, if the multivalued demand correspondence $\mathbf{x}(\mathbf{p},w)$ satisfies Walras' law and compensated law of demand (Def.\ref{thm_compensated_lawofdemand_general}), this $\mathbf{x}(\mathbf{p},w)$ satisfies the generalized WARP (Def.\ref{def_walrasian_WARP_general}), the proof follows the same logic of the single-valued version:
