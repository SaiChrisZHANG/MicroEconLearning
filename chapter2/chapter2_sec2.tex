...
\begin{description}
    ...
    Hence, we have
    $$(\mathbf{p}'-\mathbf{p})\cdot\left[\mathbf{x}(\mathbf{p}',w')-\mathbf{x}(\mathbf{p},w)\right]\geq 0$$, which is a contradiction (when $\mathbf{x}(\mathbf{p},w)\neq \mathbf{x}(\mathbf{p}',w')$) to the condition in Thm.\ref{thm_lawofdemand_to_WARP}: $(\mathbf{p}'-\mathbf{p})\cdot[\mathbf{x}(\mathbf{p}',w')-\mathbf{x}(\mathbf{p},w)] \leq 0$. 
\end{description}
