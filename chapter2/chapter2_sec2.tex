...
\end{bmatrix}
$$
where $s_{lk}(\mathbf{p},w)=\frac{\partial x_l(\mathbf{p},w)}{\partial p_k} +\frac{\partial x_l (p,w)}{\partial w}x_k(\mathbf{p},w)$. This term measures the differential change in $x_l$ (consumption of $l$) due to a differential change in $p_k$ (the price of $k$), when wealth is adjusted so that the consumer can still afford the pre-change consumption bundle.