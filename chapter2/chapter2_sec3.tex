...
\begin{enumerate}
    ...
    \textit{Proof}: since $\succsim$ is strictly convex, we have a unique $\mathbf{x}(\mathbf{p},w)$, and $v(\mathbf{p},w)=u(\mathbf{x}(\mathbf{p},w))$. We have proved $\mathbf{x}(\mathbf{p},w)$ is continuous, we know $u(\cdot)$ is continuous. Hence, $v(\mathbf{p},w)$ is continuous.
\end{enumerate}
