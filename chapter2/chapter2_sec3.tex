...
If we choose $t=1$, thus $x'_l=x_l$, we would have Euler's theorem.}, we have 
    $$\mathcal{L} = U^* - \frac{\partial u(\mathbf{x})}{\partial x_{l^*}}\cdot\frac{1}{p_{l^*}}\cdot \left(\sum_{l=1}^L p_lx_l \right) \leq U^* - \frac{\partial u(\mathbf{x})}{\partial x_{l^*}}\cdot x_{l^*} - \left(\sum_{l=1,l\neq l^*}^L \frac{\partial u(\mathbf{x})}{\partial x_{l}} x_l\right)$$
