...
\begin{theorem}{properties of $\mathbf{x}(\mathbf{p},w)$}{properties_walrasian_demand}
    \begin{enumerate}
        ...
        \end{enumerate}
        \item \textit{\textbf{continuity}}: Walrasian demand $\mathbf{x}(\mathbf{p},w)$ is continuous (preference is continuous). 
    \end{enumerate}
\end{theorem}
...
it has the following properties, closely related to those of Walrasian demand $\mathbf{x}(\mathbf{p},w)$ in \ref{thm:properties_walrasian_demand}:
...
The proof of these properties relies on the the properties of $\mathbf{x}(\mathbf{p},w)$:
\begin{enumerate}
    \item \textit{\textbf{homogeneity of degree zero}} in $(\mathbf{p},w)$: it follows the homogeneity of Walrasian demand $\mathbf{x}(\mathbf{p},w)$.
    \item \textit{\textbf{strictly increasing in $w$, nonincreasing in $p_l,\forall l$}}: this follows Walras' law $\mathbf{x}(\mathbf{p},w)\cdot \mathbf{p}=w$ and $v(\mathbf{p},w)=u(\mathbf{x}(\mathbf{p},w))$ where $u(\cdot)$ is increasing.
    ...
    \item \textit{\textbf{continuity}}: since $\succsim$ is strictly convex, we have a unique $\mathbf{x}(\mathbf{p},w)$, and $v(\mathbf{p},w)=u(\mathbf{x}(\mathbf{p},w))$. We have proved $\mathbf{x}(\mathbf{p},w)$ is continuous, we know $u(\cdot)$ is continuous. Hence, $v(\mathbf{p},w)$ is continuous.
\end{enumerate}
...
The optimal commodity vector in EMP is $h(\mathbf{p},u)\subset\mathbb{R}^L_+$, a.k.a., the Hicksian demand correspondence (function, or single-valued ones). Hicksian demand $h(\mathbf{p},u)$ parallels Walrasian demand $\mathbf{x}(\mathbf{p},w)$, hence it has parallel properties as those of $\mathbf{x}(\mathbf{p},w)$ in Thm.\ref{thm:properties_walrasian_demand}:
\begin{theorem}{properties of $h(\mathbf{p},u)$}{properties_hicksian_demand}
    \begin{enumerate}
        \item \textit{\textbf{homogeneity of degree zero}} in $\mathbf{p}$: $\mathbf{x}(\alpha\mathbf{p},\alpha w)=\mathbf{x}(\mathbf{p},w),\forall \mathbf{p}\gg 0,w>0,\alpha>0$
        \item \textit{\textbf{Walras' law}}: $\mathbf{p}\cdot \mathbf{x}(\mathbf{p},w)=w$
        \item \textit{\textbf{convexity/uniqueness}}:
        \begin{enumerate}
            \item[(a)] if $\succsim$ is convex, then $u(\cdot)$ is quasiconcave, hence $\mathbf{x}(\mathbf{p},w)$ is a convex set
