...
\begin{enumerate}
    \item \textit{\textbf{homogeneity of degree zero}} in $(\mathbf{p},w)$: $\mathbf{x}(\alpha\mathbf{p},\alpha w)=\mathbf{x}(\mathbf{p},w),\forall \mathbf{p}\gg 0,w>0,\alpha>0$
    \item \textit{\textbf{strictly increasing in $w$, nonincreasing in $p_l,\forall l$}}: $\mathbf{p}\cdot \mathbf{x}(\mathbf{p},w)=w$. This is guaranteed by the local nonsatiation assumption.
    \item \textit{\textbf{quasiconvexity}}:
    \item \textit{\textbf{continuity}}:
\end{enumerate}
