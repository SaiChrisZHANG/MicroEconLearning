...
With the definition of rational $\succsim$ in Def. \ref{def:succsim_rational}, we can prove the following properites of $\succ$ and $\sim$ \textit{induced} by $\succsim$:
...
Notice that negative positivity in Def. \ref{def:rational_succ}, is logically equivalent to its \textit{contrapositive}: 
$\exists z\in X\setminus\{x,y\}$ s.t. $x\not\succ z \land z\not\succ y\Rightarrow x\not\succ y$. This is percisely why 
the definition is called negative transitivity.