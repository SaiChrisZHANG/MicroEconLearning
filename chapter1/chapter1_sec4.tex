...
\end{enumerate}
With this induction, we prove Thm.\ref{thm_rational_has_utility}\footnote{Another way of proof is: Start with $x\in X$, Step1: define $W_{x}=\{y:y\prec x\}$, Step2: $W_x$ is either empty or not, if not empty, pick $\tilde{x}\in W_{x}$, repeat till Step3: define $u(a)=0$ if $a\sim x^{stop}$ where $x^{stop}$ is the “lower bound” of the utility. With this process, we can generate a utility function for any finite set $X$ that is rational}.